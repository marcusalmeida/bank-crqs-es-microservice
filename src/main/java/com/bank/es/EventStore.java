...
import static com.bank.es.Constants.AGGREGATE_ID;
import static com.bank.es.Constants.AGGREGATE_TYPE;
import static com.bank.es.Constants.DATA;
import static com.bank.es.Constants.EVENT_TYPE;
import static com.bank.es.Constants.METADATA;
import static com.bank.es.Constants.TIMESTAMP;
import static com.bank.es.Constants.VERSION;

import com.bank.es.exceptions.AggregateNotFoundException;
...
public class EventStore implements EventStoreDB {
    ...
    private static final String HANDLE_CONCURRENCY_QUERY =
            "SELECT aggregate_id FROM events e WHERE e.aggregate_id = :aggregate_id LIMIT 1 FOR UPDATE";
    ...
    private static final String EXISTS_QUERY =
            "SELECT aggregate_id FROM events WHERE e e.aggregate_id = :aggregate_id";
    ...
    public List<Event> loadEvents(
            @SpanTag("aggregateId") String aggregateId, @SpanTag("version") long version) {
    ...
    public <T extends AggregateRoot> void save(T aggregate) {
        ...
        final List<Event> aggragateEvents = new ArrayList<>(aggregate.getChanges());
        ...
        eventBus.publish(aggragateEvents);

        log.info("(save) save aggregate: {}", aggregate);
    }
        ...
        int updateResult =
                jdbcTemplate.update(
                        ...
                        Map.of(
                                ...
                                DATA,
                                Objects.isNull(snapshot.getData())
                                        ? new byte[] {}
                                        : snapshot.getData(),
                                METADATA,
                                Objects.isNull(snapshot.getMetaData())
                                        ? new byte[] {}
    ...
    @NewSpan
    public <T extends AggregateRoot> T load(
    ...
    public Boolean exists(@SpanTag("aggregateId") String aggregateId) {
        try {
            final var id =
                    jdbcTemplate.queryForObject(
                            EXISTS_QUERY, Map.of(AGGREGATE_ID, aggregateId), String.class);
    ...
    private <T extends AggregateRoot> T getAggregate(
            @SpanTag("aggregateId") final String aggregateId,
    ...
    private <T extends AggregateRoot> T getSnapshotFromClass(
            @SpanTag("snapshot") Optional<Snapshot> snapshot,
            @SpanTag("aggregateId") String aggregateId,
            @SpanTag("aggregateType") Class<T> aggregateType) {
        if (snapshot.isEmpty()) {
            final var defaultSnapshot =
                    EventSourcingUtils.snapshotFromAggregate(
                            getAggregate(aggregateId, aggregateType));
